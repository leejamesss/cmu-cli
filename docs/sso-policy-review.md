...
- State is re-filtered both on save and read. No origins/localStorage/IndexedDB,
  ...
  narrow, but **their sufficiency is not yet validated against SIO**.
...
2. **Real-engine request isolation remains unverified.** Synthetic route doubles
   ...
   those tests pass.
3. **Successful session and cookie scope remain unverified.** No authenticated
   SIO page was loaded. A later user-authorized manual login must verify which
   exact host/path/name cookies are necessary, the final page identity, and that
   login termination is not confused with an unauthenticated same-URL page.
   Reads redirected to the IdP currently fail closed as transport failure; refine
